...
1. **Scaricamento in parallelo (EPG)**: Lo script interroga direttamente le API pubbliche e non protette di Sky Italia. Per velocizzare il processo, lo script esegue le richieste HTTP per ciascun canale in parallelo tramite un `ThreadPoolExecutor` (fino a 8 connessioni simultanee), completando il download di una settimana di palinsesto per i canali configurati in circa 1-2 secondi.
...
5. **Compilazione PDF**: Il codice Typst viene salvato in un file temporaneo (`guida.typ`) e compilato in PDF invocando il comando `typst compile`. Al termine, il file temporaneo viene eliminato.
...
1. Aprire il link sopra nel browser o utilizzare uno strumento come `curl`.
